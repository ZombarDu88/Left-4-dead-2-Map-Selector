...
def welcome_user(ver_file="ver.txt", build_version="v1.3-07.09.2023-11PM"):
...
class very_usseful_func:
    ...
    def __init__(self):
        self.max_res = ctypes.windll.user32.GetSystemMetrics(0), ctypes.windll.user32.GetSystemMetrics(1)
        self.center_res = ctypes.windll.user32.GetSystemMetrics(0)/2, ctypes.windll.user32.GetSystemMetrics(1)/2
    ...
    def screen_center_res(self):
        gui.moveTo(self.center_res)

    def screen_max_res(self):
        gui.moveTo(self.max_res)
...
class spin_return_txt:
    ...
    def __init__(self,
                ...
                Wrong_setting_txt : str ="is not correct input for setting.txt correct is number(in seconds)"
    ...
    def time_txt_read(*args):
        ...
        try:
            if os.path.isfile(spin_return_txt().Time_file):

                with open(spin_return_txt().Time_file, "r" , encoding="utf-8") as action_time:
                    verif_time_for_action = action_time.read()

                if verif_time_for_action.find(","):
                    fix_time = verif_time_for_action.replace("," , ".")
                    with open(spin_return_txt().Time_file, "w" , encoding="utf-8") as action_time:
                ...
                with open(spin_return_txt().Time_file, "a", encoding="utf-8") as action_time:
        ...
        if is_launched.find(GAME) != -1:
            ...
            while True:
                ...
                if window.isMaximized == True:
                    ...
                    with open(spin_return_txt().Time_file, "a",
    ...
    def spin():
        ...
        if is_launched.find(GAME) != -1:
            ...
            very_usseful_func().screen_max_res()
            ...
            time.sleep(1.5)
            Python_Ui.activate()
            very_usseful_func().screen_center_res()
