...
def Time_txt_read(*args, File_not_found_message="The setting.txt file has been created or recreated with the time by default (5)",
	...
	if os.path.isfile(Time_file):
		with open(Time_file, "r" , encoding="utf-8") as Action_Time:
			...
			Time_txt_on_gui = "setting=",Time_for_action
			print("Time set on txt", Time_for_action)
			Var_setting_txt.set(Time_txt_on_gui)
...
def Spin_result_gui(*args):
	Var_last_randrange_maps.set("Spin= None")
...
def Start_L4D2(Succes_start_cmd="\033[1;32mSucces detect: left4dead2.exe\n\033[1;32mleft4dead2.exe is succefully started\033[0;37m", 
	...
	if L4D2_is_launched.find(Game) != -1:
		...
		if Steam_status.find(Laucher) != -1:
			print(Steam_ON)
			print("Whait... test if left4dead2.exe is launched in 10 tries:")
...
def Spin(Pyautogui_input="down"):
	...
	if L4D2_is_launched.find(Game) != -1:
		...
		maps_result_gui = "Spin=",str(maps)+"/90"
...
print("Welcome (v1.2-09.02.2023-00AM)\n------------------------------\n")
os.system("color")
...
Var_setting_txt = tkinter.StringVar()
Var_setting_txt.trace("r", Time_txt_read)
label_txt = tkinter.Label(mainapp, bg="grey", textvariable=Var_setting_txt)
label_txt.pack()

Var_last_randrange_maps = tkinter.StringVar()
Var_last_randrange_maps.trace("r", Spin_result_gui)
label_txt = tkinter.Label(mainapp, bg="grey", textvariable=Var_last_randrange_maps)
label_txt.pack()
