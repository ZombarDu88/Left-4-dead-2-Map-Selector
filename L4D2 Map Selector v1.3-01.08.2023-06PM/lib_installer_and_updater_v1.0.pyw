...
def libs_installer():
    ...
    try:
        pip_packages = os.popen("pip list").read()
            ...
            messagebox.showerror("ERROR... pip is not installed or idk")
...
def current_version_file(*args, ver_file="ver.txt", current_ver_file="v1.3-01.08.2023-06PM"):
...
def updater(url="https://github.com/ZombarDu88/Left-4-dead-2-Map-Selector/archive/refs/heads/updater_folder.zip", 
    ...
    try:
        pip_package = os.popen("pip list").read()

    except Exception:
            messagebox.showerror("ERROR... pip is not installed or idk")

    if pip_package.find("requests") != -1:
        update = requests.get(url)
        open(name_folder, "wb").write(update.content)
        messagebox.showinfo("Download update...",
        "Updated files are succesfully installed in -> \n\n"+os.getcwd()+"\n\nyou can extract and replace files install")
        ...
        open(name_folder, "wb").write(update.content)
        messagebox.showinfo("Download update...",
        "installing lib and updated file are succesfully installed in -> \n\n"+os.getcwd()+"\n\nyou can extract and replace files install")
