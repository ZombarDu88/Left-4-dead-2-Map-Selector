...
def Time_txt_read(*args, File_not_found_message="The setting.txt file has been created or recreated with the time by default (5)",
	...
	Wrong_resolution="Setting.txt input has been changed with the time by default (5)", Default_time="Time for spin and return to games (Esc) = 5", Txt_seek=42):
	...
	try:
		...
		if os.path.isfile(Time_file):
			...
			with open(Time_file, "r" , encoding="utf-8") as Action_Time: 
				Time_for_action = Action_Time.read()
			Fix_Time = Time_for_action.replace("," , ".")
...
print("Welcome",getpass.getuser(),"to L4D2 Map Selector(v1.2-09.02.2023-00AM)\n-------------------------------------------------\n")
os.system("color")
